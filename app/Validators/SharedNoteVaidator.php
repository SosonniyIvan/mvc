...
class SharedNoteValidator extends BaseValidator
{
    ...
    protected function isUserExists(array $fields): bool
    {
        ...
        if (!$exists){
            $this->setError('user_id', 'The user with id = ' . $fields['user_id'] . 'does not exist');
        }
    ...
    {
        $alreadyShared = SharedNote::where('user_id', '=', $fields['user_id'])->andWhere('note_id', '=', $fields['note_id'])->exists();
        if ($alreadyShared){
