...
class SharedNotesController  extends BaseApiController
{
    ...
    public function add(int $note_id): array
    {
        $data = [
            'note_id' => $note_id,
            ...requestBody()
    ...
    public function remove(int $note_id): array
    {
        $data = [
            'note_id' => $note_id
        ];
        ...
        if ($this->validator->validate($data) && !$this->validator->isNoteSharedWithUser($data) && $sharedNote = SharedNote::create($data)){
            $result = SharedNote::where('user_id', '=', $data['user_id'])->andWhere('note_id', '=', $data['note_id'])->delete();
            return $this->response(body: ['result' => $result ? 'Removed' : 'Failed']);
        }
